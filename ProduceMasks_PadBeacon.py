    ...
    if mirror:    
        ...
        if secondhalf_pt<0:
            ...
            if within_len != 0:
                ...
                strike = 1;
                print('segmentation mas is too big to the left for: ', s['filepath'])
        ...
        if firsthalf_pt>checksize:
            ...
            if within_len != 0:
                ...
                if strike:
                    ...
                    with open('../masks-ade-PadBeacon/SegMaskTooBig.csv', 'a') as f:
                        f.write("%s,%s\n"%(s['category'],s['filepath']))
                    image = []
                    return image
                else: 
...
def SaveMask(s,image):
    ...
    SaveMaskPath = ['..','masks-ade-PadBeacon', 'PaddedBeacon']
...
def load_ob_mask(atr,s):
    ...
    if (atr.loc[s['category'],col_parts] == 1).all(): #if it's a "part" object (such as mouse) then column 1 has nonzero, then load the parts image
        ...
        if not os.path.isfile(s['filepath']+'_parts_'+str(2)+'.png'):
            ...
            with open('../masks-ade-PadBeacon/SegMasksNotFound.csv', 'a') as f:
                for key in s.keys():
                    f.write("%s,%s\n"%(key,s[key]))
            return new_mask
        else:
